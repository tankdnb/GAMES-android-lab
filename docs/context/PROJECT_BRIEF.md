...
- As of `2026-05-11`, `17` completed GitHub research batches are documented under `research/batches/`.
- `23` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `21` `accepted` and `2` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest accepted `kotcity/kotcity` batch.
