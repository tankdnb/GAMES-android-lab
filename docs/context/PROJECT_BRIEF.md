...
- As of `2026-05-11`, `15` completed GitHub research batches are documented under `research/batches/`.
- `21` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `19` `accepted` and `2` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest accepted `sreich/ore-infinium` batch.
