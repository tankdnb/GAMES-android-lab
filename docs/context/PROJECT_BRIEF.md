...
- As of `2026-06-04`, `33` completed GitHub research batches are documented under `research/batches/`.
- `39` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `35` `accepted` and `4` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `RajashekarRaju/hangman-compose` batch.
