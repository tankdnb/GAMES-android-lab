...
- As of `2026-06-15`, `86` completed GitHub research batches are documented under `research/batches/`.
- `92` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `77` `accepted` and `15` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `nekzabirov/IGaming-Game-Engine` batch.
