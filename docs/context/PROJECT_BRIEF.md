...
- As of `2026-06-12`, `72` completed GitHub research batches are documented under `research/batches/`.
- `78` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `66` `accepted` and `12` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `The-JDdev/Heroes-Arena` batch and the latest accepted `jrenner/kotlin-voxel` batch.
