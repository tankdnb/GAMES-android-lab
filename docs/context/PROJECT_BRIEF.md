...
- As of `2026-06-04`, `64` completed GitHub research batches are documented under `research/batches/`.
- `70` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `61` `accepted` and `9` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `Saar25/PlanetEngine` batch.
