...
- As of `2026-06-04`, `44` completed GitHub research batches are documented under `research/batches/`.
- `50` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `44` `accepted` and `6` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `codeyousef/Materia` batch.
