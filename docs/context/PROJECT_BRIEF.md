...
- As of `2026-06-15`, `81` completed GitHub research batches are documented under `research/batches/`.
- `87` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `72` `accepted` and `15` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `AxieFeat/Arc` batch.
