...
- As of `2026-06-15`, `87` completed GitHub research batches are documented under `research/batches/`.
- `93` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `77` `accepted` and `16` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `masafykun/GamesHubAndroid` batch and the latest accepted `nekzabirov/IGaming-Game-Engine` batch.
