...
- As of `2026-06-04`, `60` completed GitHub research batches are documented under `research/batches/`.
- `66` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `58` `accepted` and `8` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `aleksrutins/platinum` batch and the latest accepted `alexvanyo/composelife` batch.
