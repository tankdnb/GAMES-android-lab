...
- As of `2026-06-04`, `62` completed GitHub research batches are documented under `research/batches/`.
- `68` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `59` `accepted` and `9` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `tottelofstrom/NFC-DOOM` batch.
