...
- As of `2026-07-10`, `91` completed GitHub research batches are documented under `research/batches/`.
- `97` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `80` `accepted` and `17` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `Koishi-Satori/KStg` batch and the latest accepted `digorydoo/titanium` batch.
