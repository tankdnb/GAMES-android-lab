...
- As of `2026-06-04`, `54` completed GitHub research batches are documented under `research/batches/`.
- `60` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `53` `accepted` and `7` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `xxxcucus/planes` batch.
