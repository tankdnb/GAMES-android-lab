...
- As of `2026-06-04`, `36` completed GitHub research batches are documented under `research/batches/`.
- `42` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `37` `accepted` and `5` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `CheerWizard/Kanvas` batch and the latest accepted `StudioAdriatic/PGSGP` batch.
