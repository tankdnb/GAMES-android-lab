...
- As of `2026-07-11`, `93` completed GitHub research batches are documented under `research/batches/`.
- `99` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `80` `accepted` and `19` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `amirisback/piano-tiles-clone` batch and the latest accepted `digorydoo/titanium` batch.
