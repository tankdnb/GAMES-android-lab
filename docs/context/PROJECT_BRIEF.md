...
- As of `2026-06-04`, `58` completed GitHub research batches are documented under `research/batches/`.
- `64` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `57` `accepted` and `7` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `rogal01/tower-defense-android` batch.
