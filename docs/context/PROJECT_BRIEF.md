...
- As of `2026-06-04`, `46` completed GitHub research batches are documented under `research/batches/`.
- `52` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `46` `accepted` and `6` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `libgdx/gdx-liftoff` batch.
