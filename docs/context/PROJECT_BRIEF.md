...
- As of `2026-06-12`, `73` completed GitHub research batches are documented under `research/batches/`.
- `79` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `67` `accepted` and `12` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `DominicDolan/Mechanica` batch.
