...
- As of `2026-06-05`, `68` completed GitHub research batches are documented under `research/batches/`.
- `74` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `64` `accepted` and `10` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `AndreasHefti/flyko-lib` batch.
