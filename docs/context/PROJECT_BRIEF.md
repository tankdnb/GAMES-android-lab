...
- As of `2026-06-15`, `80` completed GitHub research batches are documented under `research/batches/`.
- `86` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `71` `accepted` and `15` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `MohamedRejeb/Card-Game-Animation` batch and the latest accepted `soyuz-dev/KotlinGameEngine` batch.
