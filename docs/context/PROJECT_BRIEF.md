...
- As of `2026-06-04`, `37` completed GitHub research batches are documented under `research/batches/`.
- `43` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `38` `accepted` and `5` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `queuejw/Space` batch.
