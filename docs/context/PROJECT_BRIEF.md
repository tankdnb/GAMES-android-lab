...
- As of `2026-06-04`, `57` completed GitHub research batches are documented under `research/batches/`.
- `63` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `56` `accepted` and `7` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `Amigoconglomeration918/LinkGame` batch.
