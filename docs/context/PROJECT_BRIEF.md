...
- As of `2026-06-04`, `61` completed GitHub research batches are documented under `research/batches/`.
- `67` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `58` `accepted` and `9` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched `yaroslavzghoba/KotCore` batch and the latest accepted `alexvanyo/composelife` batch.
