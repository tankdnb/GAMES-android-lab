...
- As of `2026-06-04`, `48` completed GitHub research batches are documented under `research/batches/`.
- `54` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `47` `accepted` and `7` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and accepted `edezadev/la-bomba` batch.
