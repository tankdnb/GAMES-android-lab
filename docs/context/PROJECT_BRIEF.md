...
- As of `2026-06-04`, `63` completed GitHub research batches are documented under `research/batches/`.
- `69` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- The current registry split is `60` `accepted` and `9` `reference-only`.
- The public catalog and findings set cover the current recorded repository set, including the latest researched and latest accepted `mimoguz/tripeaks-gdx` batch.
