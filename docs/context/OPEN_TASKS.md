...
- Prepare the next lightweight mixed batch from backlog candidates and fresh GitHub search results.
...
- Prefer backlog candidates such as `TobseF/Candy-Crush-Clone` and `AntonioNoack/RemsEngine`, plus one or two newly searched repositories that are not yet in the registries.
