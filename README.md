...
- As of `2026-06-15`, `85` real GitHub research batches are completed end-to-end.
- `91` researched repositories are recorded in `research/registry/RESEARCHED_REPOS.md`.
- Current registry split:
  - `accepted`: `76`
  - `reference-only`: `15`
  - `partial`: `0`
  - `rejected`: `0`
- The latest researched addition is `HighviewOne/PopItBubble` as an `accepted` Android-native micro-game reference whose main reusable value is its custom `Canvas` playfield, multi-touch popping, generated `SoundPool` effects, and lightweight challenge/settings shell.
- The latest accepted addition is `HighviewOne/PopItBubble`.
